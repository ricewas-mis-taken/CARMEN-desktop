            ...
            if status["isActive"] and not status["isPaused"] and not status["isBreak"]:
                ...
                if session_manager.get_lock_mode() == "hard":
                    ...
                    for swept_process, _swept_hwnd in swept:
                        ...
                        # notice, so calling it for every swept window (even
                        # several of the same process, or one already
                        # notified via the foreground path this same tick)
                        # never produces more than one popup per violation.
